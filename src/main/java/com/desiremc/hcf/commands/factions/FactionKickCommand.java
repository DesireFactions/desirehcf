...
public class FactionKickCommand extends FactionValidCommand
{
    ...
    public void validFactionRun(FSession sender, String[] label, List<CommandArgument<?>> arguments)
    {
        ...
        faction.broadcast(DesireHCF.getLangHandler().renderMessage("factions.kick.all", "{player}", sender.getName(), "{target}", target.getName()));
        DesireHCF.getLangHandler().sendRenderMessage(target.getSender(), "factions.kick.target", "{faction}", faction.getName(), "{player}", sender.getName());
    }
}
